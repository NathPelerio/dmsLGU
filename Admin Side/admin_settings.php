...
<head>
    ...
    .stamp-modal-overlay { position: fixed; inset: 0; z-index: 2050; display: none; align-items: center; justify-content: center; padding: 1rem; background: rgba(0,0,0,0.45); }
    .stamp-modal-overlay.stamp-modal-open { display: flex; }
    .stamp-modal { background: #fff; border-radius: 12px; width: 100%; max-width: 760px; max-height: 92vh; overflow: hidden; display: flex; flex-direction: column; box-shadow: 0 20px 40px rgba(0,0,0,0.22); }
    ...
    .stamp-control-grid { display: none; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 10px; }
    .stamp-control-grid label { display: grid; gap: 5px; font-size: 12px; color: #475569; }
    .stamp-control-grid input[type="range"] { width: 100%; }
    .stamp-control-value { font-size: 12px; color: #334155; font-weight: 600; text-align: right; }
...
<body>
        ...
        <div class="main-content">
            ...
            <div class="content-body">
                ...
                <div class="settings-card">
                    ...
                    <p class="card-desc">Upload the stamp image used when sending documents to Super Admin. Position and size are adjusted before each send.</p>
    ...
    <div class="stamp-modal-overlay" id="stamp-modal-overlay">
        <div class="stamp-modal">
            ...
            <div class="stamp-modal-body">
                ...
                <div class="stamp-control-grid">
                    <label>Size
                        <input type="range" id="stamp-size-range" min="5" max="60" step="1">
                        <span class="stamp-control-value" id="stamp-size-val">18%</span>
                    </label>
                    <label>Horizontal
                        <input type="range" id="stamp-x-range" min="5" max="95" step="1">
                        <span class="stamp-control-value" id="stamp-x-val">82%</span>
                    </label>
                    <label>Vertical
                        <input type="range" id="stamp-y-range" min="5" max="95" step="1">
                        <span class="stamp-control-value" id="stamp-y-val">84%</span>
                    </label>
                </div>
            </div>
    ...
    <form method="post" id="stamp-update-form" action="admin_settings.php" style="display:none;">
        <input type="hidden" name="action" value="update_stamp">
        <input type="hidden" name="stamp" id="stamp-hidden-input">
        <input type="hidden" name="stamp_width_pct" id="stamp-width-hidden-input" value="<?= (float)($userStampCfg['width_pct'] ?? 18) ?>">
        <input type="hidden" name="stamp_x_pct" id="stamp-x-hidden-input" value="<?= (float)($userStampCfg['x_pct'] ?? 82) ?>">
    ...
    (function(){
        ...
        var editorStage = document.getElementById('stamp-editor-stage');
        var editorImage = document.getElementById('stamp-editor-image');
        var saveBtn = document.getElementById('stamp-save-btn');
        var sizeRange = document.getElementById('stamp-size-range');
        var xRange = document.getElementById('stamp-x-range');
        var yRange = document.getElementById('stamp-y-range');
        var sizeVal = document.getElementById('stamp-size-val');
        var xVal = document.getElementById('stamp-x-val');
        var yVal = document.getElementById('stamp-y-val');
        ...
        var sizePct = parseFloat(<?php echo json_encode((float)($userStampCfg['width_pct'] ?? 18)); ?>) || 18;
        var posXPct = parseFloat(<?php echo json_encode((float)($userStampCfg['x_pct'] ?? 82)); ?>) || 82;
        var posYPct = parseFloat(<?php echo json_encode((float)($userStampCfg['y_pct'] ?? 84)); ?>) || 84;

        function clamp(v, min, max){ return Math.max(min, Math.min(max, v)); }
        function syncUi() {
            sizePct = clamp(sizePct, 5, 60);
            posXPct = clamp(posXPct, 5, 95);
            posYPct = clamp(posYPct, 5, 95);
            if (sizeRange) sizeRange.value = String(sizePct);
            if (xRange) xRange.value = String(posXPct);
            if (yRange) yRange.value = String(posYPct);
            if (sizeVal) sizeVal.textContent = Math.round(sizePct) + '%';
            if (xVal) xVal.textContent = Math.round(posXPct) + '%';
            if (yVal) yVal.textContent = Math.round(posYPct) + '%';
            if (editorImage) {
                if (stampData) {
        ...
        [sizeRange, xRange, yRange].forEach(function(el){
            if (!el) return;
            el.addEventListener('input', function(){
                sizePct = parseFloat(sizeRange && sizeRange.value || sizePct) || sizePct;
                posXPct = parseFloat(xRange && xRange.value || posXPct) || posXPct;
                posYPct = parseFloat(yRange && yRange.value || posYPct) || posYPct;
                syncUi();
            });
        });

        if (editorImage && editorStage) {
            var dragging = false;
            function moveAt(clientX, clientY) {
                var rect = editorStage.getBoundingClientRect();
                var x = ((clientX - rect.left) / rect.width) * 100;
                var y = ((clientY - rect.top) / rect.height) * 100;
                posXPct = clamp(x, 5, 95);
                posYPct = clamp(y, 5, 95);
                syncUi();
            }
            editorImage.addEventListener('mousedown', function(e){ if (editorImage.hidden) return; dragging = true; e.preventDefault(); });
            window.addEventListener('mousemove', function(e){ if (!dragging) return; moveAt(e.clientX, e.clientY); });
            window.addEventListener('mouseup', function(){ dragging = false; });
            editorImage.addEventListener('touchstart', function(e){ if (editorImage.hidden) return; dragging = true; e.preventDefault(); }, { passive: false });
